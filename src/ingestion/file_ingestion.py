...
def _fichier_deja_importe(fichier_hash: str) -> bool:
    """
    Version simplifiée : vérifie si un fichier existe déjà dans data/bronze/
    avec le même contenu (hash). Pas besoin de PostgreSQL.
    """
    # Vérifier s'il existe un fichier avec ce hash (stocké dans un fichier .hash)
    hash_file = DATA_BRONZE_DIR / "imports_hash.txt"
    if not hash_file.exists():
        return False
    
    try:
        with open(hash_file, 'r') as f:
            imported_hashes = set(line.strip() for line in f)
        return fichier_hash in imported_hashes
    except Exception:
        return False


def _enregistrer_hash_importe(fichier_hash: str) -> None:
    """Enregistre le hash du fichier importé pour éviter les doublons."""
    hash_file = DATA_BRONZE_DIR / "imports_hash.txt"
    with open(hash_file, 'a') as f:
        f.write(f"{fichier_hash}\n")
...
def ingest_file(chemin_fichier, deplacer_vers_bronze: bool = True) -> ResultatIngestion:
    ...
    # En version simplifiée, on simule le chargement en enregistrant le hash
    _enregistrer_hash_importe(fichier_hash)
    
    # Simuler un résultat de chargement
    resultat = {
        "inserees": len(df_valide),
        "ignorees": 0,
        "lues": len(df_brut)
    }
